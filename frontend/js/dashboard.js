...
export async function saveDoctorDecision(escalationId, decision, note = "") {
  if (!escalationId) {
    throw new Error("Select an escalated case before saving a triage decision.");
  }

  const response = await fetch(`${API_BASE}/doctor/escalations/${escalationId}/decision`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ decision, note }),
  });
  return await readJsonResponse(response, "Unable to save triage decision.");
}
...
export async function fetchAdminCases() {
  try {
    return await adminRequest("/admin/cases");
  } catch (error) {
    if (!isMissingRouteError(error)) throw error;
    return await fetchTriageQueue();
  }
}

export async function fetchAdminAuditLog() {
  try {
    return await adminRequest("/admin/audit-log");
  } catch (error) {
    if (!isMissingRouteError(error)) throw error;
    return await buildFallbackAuditLog();
  }
}
...
async function buildFallbackAuditLog() {
  const events = [];
  const [users, predictions, cases] = await Promise.allSettled([
    fetchAdminUsers(),
  ...
  if (users.status === "fulfilled") {
    users.value.forEach((user) => {
      events.push({
        type: "User",
        title: `${user.role || "User"} account: ${user.full_name || user.email || `User #${user.id}`}`,
        description: `Approval status: ${user.role_status || "approved"}`,
        created_at: null,
      });
    });
  }
  ...
  if (predictions.status === "fulfilled") {
    predictions.value.forEach((item) => {
      events.push({
        type: "Prediction",
        title: `Case #${item.id}: ${item.full_name || `Patient #${item.id}`}`,
        description: `${item.risk_level || "Unknown"} risk, ${formatRiskPercent(item.risk_probability)} probability`,
  ...
  if (cases.status === "fulfilled") {
    cases.value
      .filter((item) => item.escalated || item.status === "Escalated" || item.status === "Reviewed")
      .forEach((item) => {
        events.push({
          type: "Escalation",
          title: `Case #${item.id}: ${item.patient_name || item.full_name || "Patient case"}`,
          description: item.doctor_decision || item.status || "Escalated to doctor",
          created_at: item.reviewed_at || item.escalated_at || item.created_at || null,
        });
      });
  }
